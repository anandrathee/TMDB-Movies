...
const Button = ({ text, text2, handleBtn, type = "toggle" }) => {
  ...
  if (type === "toggle") {
    ...
    return (
      <div 
        onClick={handleClick}
        className="border border-white rounded-full flex items-center justify-between w-52 h-8 px-4 relative cursor-pointer select-none"
      >
        <div
          className={`bg-zinc-600 text-[#64E6B9] font-semibold flex items-center justify-center w-25.5 h-6 rounded-full absolute left-0 transition-all duration-200 ${istrue ? "translate-x-full" : ""}`}
        >
