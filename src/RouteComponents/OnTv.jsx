...
const OnTv = () => {
  ...
  const getOnTv = async () => {
    ...
    try {
      setLoading(true);
      const endpoint = `/tv/on_the_air?page=${currentPage}`;
      const { data } = await axios.get(endpoint);
      
      //  DUPLICATE CHECK + FILTER
      const newMovies = data.results.filter(
        newMovie => !onTv.some(existing => existing.id === newMovie.id)
      );
      
      setOnTv(prev => [...prev, ...newMovies]);
      ...
      console.log(`Page ${currentPage}: ${newMovies.length} new movies (total: ${onTv.length + newMovies.length})`);
    } catch (error) {
  ...
  return (
    <div className="w-full h-full flex flex-col px-32.5 py-10 gap-6 bg-zinc-800">
      ...
      <div className="movies flex flex-wrap gap-10 items-start justify-start">
        {onTv.map((elem, index) => (
          <div 
            ...
            ref={index === onTv.length - 1 && hasMore ? lastMovieRef : null}
          >
      ...
      {/* End message */}
      {!hasMore && onTv.length > 0 && (
        <div className="w-full text-center py-12">
          <p className="text-gray-400 text-xl">🎉 All {onTv.length} movies loaded!</p>
        </div>
      )}
    </div>
  );
